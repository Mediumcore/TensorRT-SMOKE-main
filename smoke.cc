...
void SMOKE::Detect(const cv::Mat &raw_img, cv::Mat &output_img)
{
    // Preprocessing
    cv::Mat img_resize;
    ...
    PostProcess(img_resize, output_img);
}
...
void SMOKE::PostProcess(cv::Mat &input_img, cv::Mat &output_img)
{
    ...
    cv::resize(input_img, output_img, cv::Size(IMAGE_W, IMAGE_H), cv::INTER_LINEAR);

    // cv::imwrite("SMOKE_TRT.png", input_img);
